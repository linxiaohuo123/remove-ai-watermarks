...
def _load_visible_input(
    ...
    *,
    ...
    ``alpha`` / ``provenance`` let a caller that already holds the normalized
    pixels (the GUI pipeline's single-decode truth) pass them in instead of the
    file being re-decoded and re-scanned here.
    """
    if not isinstance(source, (str, Path)):
        return _VisibleInput(source, alpha=alpha, provenance=provenance or frozenset())

    from remove_ai_watermarks import image_io

    path = Path(source)
    file_bgr, file_alpha = image_io.read_bgr_and_alpha(path)
    if file_bgr is None:
    ...
    return _VisibleInput(
        ...
        path=path,
        provenance=provenance if provenance is not None else visible_provenance(path),
    )
...
def remove_visible(
    ...
    *,
    ...
    ``alpha`` / ``provenance`` are the shared-pixel injection points: a caller that
    already decoded (and EXIF-normalized) the pixels may pass the alpha plane and the
    metadata-derived vendor keys so the file is not re-decoded/re-scanned (GUI pipeline).
    """
    ...
    loaded = _load_visible_input(source, alpha=alpha, provenance=provenance)
