...
class NormalizedDecode:
    """One decode, already EXIF-Orientation-normalized to the display direction.

    Detection, localization and removal must all consume THIS same array; the
    written output is then physically display-oriented and must carry no
    Orientation tag (or Orientation=1) or viewers would rotate it a second time.
    """

    bgr: NDArray[Any]
    alpha: NDArray[Any] | None
    orientation: int  # 1-8 as stored in EXIF (1 when absent)
    display_swapped: bool  # True when the physical pixels are transposed (5-8)


def read_normalized(path: str | Path) -> NormalizedDecode | None:
    """Decode once with EXIF Orientation applied exactly once.

    cv2's implicit EXIF handling and Pillow's differ, so re-decodings could see
    different pixel directions; the GUI pipeline uses this as the single truth.
    Returns None when the file cannot be fully decoded. Orientation is read from
    the EXIF 0th IFD; a missing/invalid value is treated as 1 (no rotation)."""
    import numpy as np
    ...
    try:
        with Image.open(path) as im:
            im.load()
            orientation = int(im.getexif().get(0x0112, 1))
            if orientation not in (1, 2, 3, 4, 5, 6, 7, 8):
                orientation = 1
            display_swapped = orientation in (5, 6, 7, 8)
            disp = ImageOps.exif_transpose(im)
            disp.load()
            rgb = np.asarray(disp.convert("RGB"))
            bgr = rgb[..., ::-1].copy()
            alpha: NDArray[Any] | None = None
            if disp.mode in ("RGBA", "LA", "PA") or "transparency" in disp.info:
                alpha = np.asarray(disp.convert("RGBA"))[..., 3].copy()
            return NormalizedDecode(bgr=bgr, alpha=alpha, orientation=orientation, display_swapped=display_swapped)
    except Exception:
        return None
