...
def _replace_jpeg_exif_segment(data: bytes, new_exif: bytes) -> bytes:
    """Swap the JPEG APP1-Exif segment's payload in place, leaving every other
    marker segment (JFIF/APP0 with its DPI density, ICC APP2, XMP, ...) and the
    DCT scan byte-identical.

    ``piexif.insert`` was the previous rewrite path and silently DROPPED the
    JFIF APP0 segment -- losing the resolution/DPI info -- while only scrubbing
    AI EXIF tags (GUI BUG-06 regression). Returns the rewritten JPEG bytes.
    """
    ...
    while i + 1 < n:
        ...
        if seg_len < 2 or seg_end > n:
            return data  # malformed: leave the file untouched rather than corrupt it
        payload = data[i + 4 : seg_end]
        if marker == 0xE1 and payload.startswith(b"Exif\x00\x00") and not inserted:
            seg = b"\xff\xe1" + (2 + len(new_exif)).to_bytes(2, "big") + new_exif
            out += seg
            inserted = True
        else:
            out += data[i:seg_end]
...
def _strip_jpeg_metadata_lossless(source_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        if _scrub_ai_exif(exif):
            new_exif = piexif.dump(exif)
            # Byte-level segment swap (NOT piexif.insert, which drops the JFIF
            # APP0 segment and with it the DPI/resolution info).
            data = output_path.read_bytes()
            output_path.write_bytes(_replace_jpeg_exif_segment(data, new_exif))
    except Exception:
...
# inconclusive (never for JPEG re-encode of lossless content). BMP/TIFF are here
# so a same-format strip writes real BMP/TIFF bytes, never PNG under a foreign
# extension (GUI BUG-02 regression); GIF is deliberately absent (the GUI rejects
# it at precheck, and the library never claims to re-encode animation).
_EXT_TO_PIL_FORMAT = {
    ".jpg": "JPEG",
    ".jpeg": "JPEG",
    ".webp": "WEBP",
...
def strip_and_verify(
    ...
    *,
    ...
    A surviving marker is reported as ``surviving`` -- the caller decides whether the
    output may ship. There is deliberately NO fallback that re-encodes the whole raster
    through a bare codec path: that recovery used to silently drop ordinary EXIF, text,
    ICC and DPI on a "successful" strip (GUI BUG-06 / PRD 5.3), which is worse than an
    explicit failure.
    ...
    logger.warning(
        "AI metadata survived stripping (%s); output kept as-is, caller must not publish: %s",
...
def remove_ai_metadata(
    ...
    with Image.open(source_path) as img:
        ...
        if exif_data and save_kwargs["format"] in ("JPEG", "PNG", "TIFF", "WEBP"):
            # Scrub AI-provenance EXIF tags (xAI/Grok signature, generator tokens)
            # while keeping genuine camera/editor EXIF; PNG/TIFF/WebP carry the same
            # eXIf semantics when their container supports an Exif block.
