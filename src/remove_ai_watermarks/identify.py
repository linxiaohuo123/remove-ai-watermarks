...
class _SharedDecode:
    ...
    * :meth:`get` RE-RAISES it. The invisible arm never caught a decode error, and
    ...
    def __init__(self, path: Path, image: NDArray[Any] | None = None) -> None:
        self._path = path
        self._image = image
        self._done = image is not None
...
def _identify_from_evidence(
    ...
    *,
    ...
    ``pixels`` pre-decoded, EXIF-normalized BGR pixels shared with a removal pass.
    """
    ...
    decode = (
        _SharedDecode(pixel_path, image=pixels)
        if pixel_path is not None
        else _SharedDecode(evidence.path, image=pixels)
    )
...
def identify(
    ...
    *,
    ...
    Args:
        ...
        pixels: Optional pre-decoded, EXIF-orientation-normalized BGR pixels. When
            given, the visible detectors consume THIS array instead of re-decoding
            the file, so a caller's report and removal share one pixel truth.
