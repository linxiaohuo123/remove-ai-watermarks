...
`maintain.sh` runs dependency freshness and security checks, Ruff, Pyright (`src/` strict plus `scripts/gui_app.py`; `tests/` is relaxed assertion code), and the parallel test suite. Full-project Pyright is not the project gate because the ML dependency graph can exhaust Node memory.
