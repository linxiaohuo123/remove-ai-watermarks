...
class TestHasAiMetadata:
    ...
    def test_strip_and_verify_reports_survivors_without_dropping_metadata(self, tmp_path: Path):
        """BUG-06 (PRD 5.3): a surviving marker is REPORTED, never papered over by
        re-encoding the raster through a bare codec path that silently discards
        ordinary EXIF/text/ICC/DPI on a "successful" strip. The caller must not
        publish an output whose strip is incomplete."""
        import numpy as np
        ...
        assert result == out
        # The undecodable C2PA marker survives and the strip is reported as
        # incomplete -- no silent normalize-and-pretend-success path exists anymore.
